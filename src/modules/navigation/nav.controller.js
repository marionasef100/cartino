...
//draw map and path 
export const drawPath =async (req,res,next)=>{

const fs = require('fs');

const firstApiResponse = await fetch('https://cartino.onrender.com/navigation');
    const firstApiData = await firstApiResponse.json();
    const valuex = firstApiData.indX;
    const valuey = firstApiData.indY;
...
const customerX = valuex;
const customerY = valuey;
ctx.fillStyle = 'blue';
ctx.beginPath();
